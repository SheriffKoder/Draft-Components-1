...
const TableFilterLight = ({ 
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {columnName}
        <div className={`ml-1 p-1 right-1/2 rounded-full ${isOpen || isFilterActive ? 'bg-primary text-white' : 'text-gray-500'}`}>
          <FilterIcon size={10} />
        </div>
      </div>
          ...
          <div className="p-2">
            {filteredValues.length > 0 ? (
              filteredValues.map(({ value, displayValue, count }) => (
                ...
                >
                  <div className={`w-4 h-4 border rounded mr-2 flex items-center justify-start ${activeFilters.includes(value) ? 'bg-primary border-primary' : 'border-gray-300'}`}>
...
export default TableFilterLight;