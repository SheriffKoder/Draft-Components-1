 ...
 * @param {Array<any>} props.filteredData - The currently filtered data
...
interface TableFilterProps {
  ...
  filteredData: any[]; // Currently filtered dataset
...
const TableFilter = ({ 
  ...
  data, // This will be the full dataset
  filteredData, // Add this new prop to receive the currently filtered data
  ...
  // Get unique values with counts from both full dataset and filtered dataset
  const uniqueValuesWithCounts = useMemo(() => {
    ...
    // Count occurrences in the filtered dataset
    filteredData.forEach(item => {
      ...
      if (displayValue) {
        filteredCounts[displayValue] = (filteredCounts[displayValue] || 0) + 1;
      }
    });
    
    // Convert to array of objects with value, displayValue, totalCount and filteredCount
    return Object.entries(counts)
      .map(([displayValue, totalCount]) => ({ 
        ...
        totalCount,
        filteredCount: filteredCounts[displayValue] || 0
      }))
      .sort((a, b) => a.displayValue.localeCompare(b.displayValue));
  }, [data, filteredData, columnKey, formatDisplayValue]);
        ...
        <div className={`absolute z-10 mt-1 w-64 ${isNumberField || isHourField || isDateField ? "max-h-[300px]" : "max-h-60"} overflow-auto bg-white border border-gray-200 rounded-md shadow-lg top-[1.5rem] ${isLastColumn ? "right-[0rem] " : ""}`}>
          ...
          <div className="p-2">
            {filteredValues.length > 0 ? (
              filteredValues.map(({ value, displayValue, totalCount, filteredCount }) => (
                <div 
                  key={value} 
                  className={`flex items-center p-2 hover:bg-gray-100 cursor-pointer ${filteredCount === 0 ? 'text-gray-400' : ''}`}
                  onClick={() => toggleFilter(value, displayValue)}
                >
                  <div className={`w-4 h-4 border rounded mr-2 flex items-center justify-center ${activeFilters.includes(value) ? 'bg-[#00e6c7] border-[#37beac]' : 'border-gray-300'}`}>
                  ...
                  <div className="text-gray-500 text-xs  rounded-full min-w-[40px] flex items-center justify-center">
                    <span className="mr-auto">(</span>
                      <span className="mx-[1px]">{filteredCount} / {totalCount}</span>
